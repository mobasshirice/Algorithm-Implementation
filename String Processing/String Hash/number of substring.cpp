...
stringhash strhash,shash;
vector<int> subStringIndex(string &str,string &s){



    strhash.GenerateFH(str,BASE[0],MOD[0]);
    shash.GenerateFH(s,BASE[0],MOD[0]);

    int n=str.size();
    int m=s.size();

    long long fhash=shash.Fhash(0,m-1);

    vector<int> res;

    for(int i=0;i<n-m+1;i++){

       long long subhash=strhash.Fhash(i,i+m-1);
...
void solve(){
    ...
    while(cin>>n){
        ...
        vector<int> res=subStringIndex(str,s);
